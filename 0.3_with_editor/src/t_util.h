...
public:
	...
	static t_string rnd_string(int length, const t_string& alphabet);
	...
	static t_string xor_encrypt(const t_string& str, char xor_key);
	static t_string xor_decrypt(const t_string& str, char xor_key);
	static t_string size_to_string(uint64_t bytes);
	static t_string datetime(const t_string& format);
	static t_string datetime();
	static t_string date();
	static t_string time();
	static void set_clipboard(const t_string& text);
